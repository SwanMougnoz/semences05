...
class PageGeneriqueTest(TestCase):
    def test_url(self):
        ...
        page = PageGenerique()
        page.viewname = 's5vitrine.home_view'
        reversed_url = page.url
        # On verifie que l'URL retournee correspond a la vue specifiee
        resolved = resolve(reversed_url)
        self.assertEqual('s5vitrine.home_view', resolved.view_name)

        # Test avec des parametres
        page = PageGenerique()
        page.viewname = "s5vitrine.content_view"
        page.params = "1"
        reversed_url = page.url
        resolved = resolve(reversed_url)
        self.assertEqual('s5vitrine.content_view', resolved.view_name)
        self.assertEqual({
            "content_id": u'1'
        }, resolved.kwargs)
